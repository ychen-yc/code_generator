package com.qxs.database.dialect.mysql;
...
public class MySQLDialect extends AbstractDialect {
	...
	static {
		...
		SQL_ALL_TABLE_NAMES = read(MySQLDialect.class,SQL_ALL_TABLE_NAMES_FILE_NAME);
		SQL_TABLE = read(MySQLDialect.class,SQL_TABLE_FILE_NAME);
		SQL_COLUMNS = read(MySQLDialect.class,SQL_COLUMNS_FILE_NAME);
	}
	...
	public List<Database> getDatabases(Connection connection, String databaseVersion) throws SQLException {
		...
		for(Map<String, Object> map : list) {
			...
			//mysql 8 版本需要过滤sys库
			if("sys".equalsIgnoreCase(databaseName) && databaseVersion.startsWith("8")){
				continue;
			}
